...
local function get_paired_lines(mapping, side)
  local paired = {}
...
function M.render_inline_diff(bufnr, diff_result, original_lines, modified_lines, opts)
  ...
  for _, mapping in ipairs(diff_result.changes) do
    ...
    local paired_original = get_paired_lines(mapping, "original")
    local paired_modified = get_paired_lines(mapping, "modified")
    ...
    if has_original then
      ...
      for orig_line = orig_start, orig_end - 1 do
        ...
        local base_hl = paired_original[orig_line] and "CodeDiffLineDelete" or "CodeDiffLineDeleteText"
        local chunks = build_highlighted_virt_line(line_text, char_ranges, line_syntax, base_hl)
    ...
    if has_modified then
      ...
      for line = mod_start, mod_end - 1 do
        ...
        if not paired_modified[line] and modified_lines[line] and #modified_lines[line] > 0 then
          pcall(vim.api.nvim_buf_set_extmark, bufnr, M.ns_inline, line_idx, 0, {
            end_col = #modified_lines[line],
            hl_group = "CodeDiffLineInsertText",
            priority = 200,
          })
        end
      end
